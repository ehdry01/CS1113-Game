...
game_name = "Escape from USS Callister!"

help_text = "To interact with this game world, you will use a basic text-based interface. \
Try single-word commands like 'inventory' or 'left' (or their counterpart abbreviations, 'i' or 'l', respectively \
to get started. For more complex interactions, use commands of the format [VERB][NOUN] (e.g. 'open door', \
or in some cases, [VERB][NOUN][OBJECT] (e.g. 'attack spacethief with spaceknife').\
...
def handle_input(verb, noun1, noun2):
	...
	if(verb == 'help'):
		...
		if(not noun2):
			if(noun1 == 'forward'):
				[move_status, move_description] = world.check_forward(player.x, player.y)
				if(move_status):
					player.move_forward()
				...
				[move_status, move_description] = world.check_back(player.x, player.y)
				if(move_status):
					player.move_back()
					world.tile_at(player.x, player.y).random_spawn()		# Randomly spawn enemies if possible.
					return [move_description, world.tile_at(player.x, player.y).intro_text()]
				...
				[move_status, move_description] = world.check_right(player.x, player.y)
				if(move_status):
					player.move_right()
					world.tile_at(player.x, player.y).random_spawn()		# Randomly spawn enemies if possible.
					return [move_description, world.tile_at(player.x, player.y).intro_text()]
				...
				[move_status, move_description] = world.check_left(player.x, player.y)
				if(move_status):
					player.move_left()
					world.tile_at(player.x, player.y).random_spawn()		# Randomly spawn enemies if possible.
					return [move_description, world.tile_at(player.x, player.y).intro_text()]
